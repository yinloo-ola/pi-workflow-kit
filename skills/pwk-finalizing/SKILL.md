...
3. Read **every** relevant progress file — for an umbrella that's each part's progress file — discovered by recursively listing `docs/plans` for `*-progress.md`, excluding docs/plans/completed/ (umbrella docs live in `docs/plans/<date>-<umbrella>/` folders — archived parts are already shipped), letting the search tool do the recursing (e.g. `find docs/plans -name '*-progress.md' -not -path '*/completed/*'`) — glob patterns like `**` don't recurse in non-interactive shells. Match the `Feature phase:` line (e.g. `grep -m1 '^Feature phase:' <file>`) and read only the matching ❌/⏭ verdict rows in the Requirements table (e.g. `grep '❌' <file>` / `grep '⏭' <file>` — rows with reasons); the sections after the tables carry nothing the gate needs. For a standalone design doc, the one:
