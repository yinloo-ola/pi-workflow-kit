...
3. Read **every** relevant progress file — for an umbrella that's each part's progress file — discovered by a recursive search under `docs/plans` excluding docs/plans/completed/ for `*-progress.md` (umbrella docs live in `docs/plans/<date>-<umbrella>/` folders — archived parts are already shipped) with any reliable recursive search — e.g. `find docs/plans -name '*-progress.md' -not -path '*/completed/*'` (POSIX) or `Get-ChildItem -Recurse docs/plans -Filter '*-progress.md'` (Windows); never shell glob wildcards (non-interactive bash's globstar is off — `**` silently degrades one level); for a standalone design doc, the one:
