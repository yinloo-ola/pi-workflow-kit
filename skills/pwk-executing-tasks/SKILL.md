...
2. **Find the doc** — first verify the repo root: run `pwd` and `git rev-parse --show-toplevel`; mismatch → report both paths and stop; never `cd` (a worktree root counts). Then run `/usr/bin/find docs/plans -name '<suffix>' -not -path '*/completed/*'`, excluding docs/plans/completed/ (Windows: `Get-ChildItem -Recurse docs/plans -Filter '<suffix>'`, minus any `completed` folder) — recursive by construction — for `*-design.md` and `*-implementation.md` (umbrella docs live in `docs/plans/<date>-<umbrella>/` folders — archived work is not pending). A stem-matched legacy `*-implementation.md` wins for that topic (an in-flight 1.x feature — old flow). If no doc at all, ask the user to run `/skill:pwk-brainstorming` first; if several, ask which. Report one line, e.g. `Found: design "auth" — feature-gate execute (feature-spec done, implementing 2/5)`. A matching `*-progress.md` means this is a **resume** (see [Resume](#resume)).
