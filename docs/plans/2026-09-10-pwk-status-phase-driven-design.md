...
- Discovery is pinned to an invariant, not a tool: a recursive search under `docs/plans` that reaches nested umbrella folders and skips `completed/` — never shell glob wildcards. Example commands (POSIX `find`, Windows `Get-ChildItem -Recurse`) are guidance the model may swap for its harness's fastest reliable recursive search. `(rejected: commands as mandates — over-specifies tooling and ages badly per-platform, and the model knows its harness better than the skill does; rejected: bare abstract verb with no invariant — the proven globstar degradation and full-file token burns came from exactly that freedom)`
...
Every skill that discovers in-flight planning artifacts states the discovery invariant — a recursive search under `docs/plans` that reaches nested umbrella folders and skips `completed/` (excluding docs/plans/completed/), never shell glob wildcards — and carries per-platform example commands (POSIX `find`, Windows `Get-ChildItem -Recurse`) that the model may replace with its harness's fastest reliable recursive search: `pwk-status` step 1 (`*-design.md`, `*-implementation.md`, `*-progress.md`, `overview.md`), `pwk-brainstorming` step 2 (`*-design.md`, `overview.md`), `pwk-executing-tasks` step 2 (`*-design.md`, `*-implementation.md`), `pwk-finalizing` pre-finalization check 2 + Process step 1 (`*-progress.md`, `overview.md`). Conditional mentions of "a `docs/plans/**/overview.md` exists" may stay as conditions; only discovery *recipes* are pinned. Recipe lines keep the phrase `excluding docs/plans/completed/` so the `completedExclusion` marker stays valid.
