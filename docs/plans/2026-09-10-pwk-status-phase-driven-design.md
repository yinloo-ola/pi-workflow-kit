...
- Discovery is pinned to an invariant stated in positive terms: recursively list files under `docs/plans`, reaching the nested umbrella folders, skipping `completed/`, letting the search tool do the recursing — with one example command as a fallback. `(rejected: enumerating per-platform command examples — extra tokens for knowledge the model already has; rejected: bare abstract verb with no invariant — the proven globstar degradation and full-file token burns came from exactly that freedom)`
...
- Given any progress file regardless of size, When status gathers state, Then it reads only the file's header (e.g. `head -n 10` — the `Feature phase:` line sits in the first 10 lines of the executor's template), takes the in-flight tally from the phase line itself (`implementing (k/N)`), counts done rows via a count command (e.g. `grep -c '✅'`) when a done tally is wanted, and stops there — the file body (execution summary, review reports, code digest) carries nothing status needs, and neither do design docs; if the line is missing from the header, `grep -m1 '^Feature phase:' <file>` finds it without a full read.
...
Every skill that discovers in-flight planning artifacts states the discovery invariant in positive terms — recursively list files under `docs/plans` reaching the nested umbrella folders and excluding docs/plans/completed/ — and carries one example command; glob patterns are simply the wrong tool for it (they don't recurse in non-interactive shells): `pwk-status` step 1 (`*-design.md`, `*-implementation.md`, `*-progress.md`, `overview.md`), `pwk-brainstorming` step 2 (`*-design.md`, `overview.md`), `pwk-executing-tasks` step 2 (`*-design.md`, `*-implementation.md`), `pwk-finalizing` pre-finalization check 2 + Process step 1 (`*-progress.md`, `overview.md`). Conditional mentions of "a `docs/plans/**/overview.md` exists" may stay as conditions; only discovery *recipes* are pinned. Recipe lines keep the phrase `excluding docs/plans/completed/` so the `completedExclusion` marker stays valid.
...
- **R2 criterion (lean wording)**: each site carries exactly one example command; platform coverage comes from the invariant itself (any platform's recursive search satisfies it), not from enumerated variants.
...
- **Invariant + example commands (chosen) vs pinned commands vs abstract verb** — the invariant (recursive, reaches nested folders, skips `completed/`, no shell glob wildcards) is what closes the failure class; the commands are examples the model may outperform with harness-native tools (ffind, ripgrep --files, Glob). A bare verb with no invariant is the original bug; mandated commands age badly per-platform.
