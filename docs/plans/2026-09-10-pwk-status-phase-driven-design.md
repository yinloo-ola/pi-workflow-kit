...
- Discovery is a pinned shell recipe (`/usr/bin/find docs/plans -name '<suffix>' -not -path '*/completed/*'`), not an abstract glob verb. `(rejected: keep the glob + add a bash-globstar warning — the trap is a property of whatever shell the harness runs, so warning about one shell doesn't fix the class; `find` is recursive by construction and harness-neutral.)`
