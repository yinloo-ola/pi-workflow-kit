...
- **One folder per topic — a single-part topic and an umbrella now share one shape (breaking for docs in flight).** Every topic's planning docs live in `docs/plans/<date>-<topic>/` rather than as flat dated files beside each other: `<leaf>-design.md`, `<leaf>-progress.md`, `<leaf>-review-packet*.md`, plus `overview.md` only when the folder holds more than one leaf. "Umbrella" is now a property (leaf count), not a second layout. The folder is the unit of creation, discovery, and disposal — `pwk-finalizing` disposes one folder whether the topic has a single leaf or many, and the per-file flat globs now run only for a pre-change (legacy) flat topic, so a leaf slug can no longer collide with an unrelated flat topic's slug. Existing flat topics stay readable, resumable, and shippable; they are never newly written. No migration is required and none is provided.
- **Both flows share one identity model.** `pwk-finalizing` no longer identifies a multi-leaf topic by a repo-wide `docs/plans/**/overview.md` search — it reads the `overview.md` beside the design doc, the rule `pwk-executing-tasks` already stated, so an unrelated sibling topic can never route another topic's finalize. `<topic>` means the folder slug in both skills (the branch and worktree convention included), so the worktree probe no longer targets a leaf. The next-part suggestion is the first roster part with no `done` progress file rather than the roster successor, since build order is advisory. The `## Parts (build order)` roster heading and item shape are lint-pinned because two skills read them.
- **`pwk-code-review` checklists aligned with the four role contracts** — steps 2–5 now carry the `Your checklist` sections from `agents/pwk-{tracing,spec,smell,hazard}-reviewer.md` (the smell list keeps its unlocked apply-fixes behavior). Three deliberate divergences, all forced by the skill's unlocked/design-doc context: the coverage table is keyed on the design doc's `### R<n>:` headings rather than packet headings, the reporting-contract lines (`file:line` evidence, the `No findings` requirement) are not ported, and the tracing line reads "the acceptance criteria and the feature E2E" because "integration tests" is stale terminology this repo bans in that file. Process and reporting sections are unchanged. The alignment is no longer on the honour system: `tests/skill-lint.mjs` pins all twelve checklist items line-complete in both the skill and the role contracts, and fails when an item is edited, deleted, or appended to on either side.
