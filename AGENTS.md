...
- **`/pwk-setup [--force]`** installs the five canonical role files into `<cwd>/.agents/agents/`. Idempotent (byte-identical files are skipped); differing files are conflicts preserved unless `--force`; symlink destinations are rejected; refused outright during brainstorm/plan phases even when the guard override is `off`. It never installs or configures a delegation provider, and it never reloads the session.
